...
abstract class AnElement extends Object {
    ...
    public String UnitV;
    public String NameV;
    static int DataType; // a tag for the type of element
...
//    }
 ...
 public void ComputeColorProj(int dim) {   // copies elementdata into the color array
    ...
    private void GNE(AnElement oldelem) // will generate new element in all time lists
    {
        AnElement ne=null;
        for (int t=0;t<Times;t++)
            ne=GNE(oldelem,ElementsAtTime(t),ProjsAtTime(t));
    ...
    public void CloneShort() {  // will generate a new element in short type, thresholds WILL BE appliled
        int prevActElem=ActiveElement;
        int ne=GenerateNewElement(AnElement.IntegerType,2,16,GetScale(ActiveElement),
                            GetOffset(ActiveElement),1.0,0.0,GetAxisNames(),GetAxisUnits());
        for (int t=0;t<Times;t++) {
...
class View5D_ extends PlugInFrame implements PlugIn, WindowListener {
   ...
   public boolean LoadImg(int NumZ) {
		...
		switch (ImageType) {
			...
			case ImagePlus.GRAY16: DataType=AnElement.IntegerType;NumBytes=2;NumBits=16;break;    // 16-bit grayscale
	...
	*/
    ...
    void showAbout() {
              IJ.showMessage("About View5D, Version V1.2.03",
...
public class View5D extends Applet {
  ...
  public static float testing123(float i) {
      System.out.print("In Java, testing123: ");System.out.println(i);
      return i;
  }

  public static float [] testing456(float [] farray) {
      System.out.print("In Java, testing456: ");System.out.println(farray[0]);
      return farray;
  }
  ...
  public void init() {
	 if (System.getProperty("java.version").compareTo("1.1") < 0)  // this viewer should work from version 1.1 on
		...
		NumBits = ParseInt("bits",false,NumBits);  
                if (NumBytes > 1) {DataType=AnElement.IntegerType;}
...
class PixelDisplay extends Panel implements MouseListener,ImageObserver,KeyListener {  // A panel displaying the multi-element contens of a single pixel
  ...
  public void plot(Graphics g) {  // Plots one or multiple spectral graphs
	...
	if (false) // (yprev > r.height/2)  // lower half  -> change also the RGB assignment
		...
		if (MyMask == e.BUTTON1_MASK)
		    data3d.MarkChannel(0);
		if (MyMask == e.BUTTON2_MASK)
		    data3d.MarkChannel(1);
		if (MyMask == e.BUTTON3_MASK)
		    data3d.MarkChannel(2);
	    }
	c1.UpdateAll();
    }
    ...
    if (PlotMode <= 1)
        ...
        if(e.VK_LEFT == e.getKeyCode())
            ...
            if ((e.VK_RIGHT == e.getKeyCode()) || (e.VK_LEFT == e.getKeyCode()))
            {
...
class PositionLabel extends Panel implements MouseListener{
  ...
  public PositionLabel(String text,ImageCanvas C1,ImageCanvas C2, ImageCanvas C3,My3DData data)
  {
    ...
    DispIntScaleOffset = new CheckboxMenuItem("Intensity, Scale, Offset: ",true);
...
/*      
  ...
  */
  ...
  void Help() {
      ...
      String newtext="Java 5D image viewer, Version V1.2.03 by Rainer Heintzmann,\nKCL, London (rainer.heintzmann@kcl.ac.uk)\n\n"+
  ...
  void CoordsChanged() {
      ...
      double sx=data3d.GetScale(0,0),sy=data3d.GetScale(0,1),sz=data3d.GetScale(0,2),se=data3d.GetScale(0,3),st=data3d.GetScale(0,4);
  ...
  if (DispIntScaleOffset.getState()) {
      newtext = newtext+GetValueString()+", scale :" + nf.format(data3d.GetValueScale(elem))+", offset: "+data3d.GetValueOffset(elem)+"\n";}
...
public void keyPressed(KeyEvent e) {
    ...
    if(e.VK_PAGE_DOWN == e.getKeyCode())
    {
        if (e.isShiftDown())
            myChar = '.';           // move along time direction
        else
